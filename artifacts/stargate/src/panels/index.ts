...
export async function registerPanelCommands(client: Client) {
  ...
  if (!token) throw new Error("STARGATE_TOKEN is missing");
  ...
  const helpCommand = new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName("verification").setDescription("How the verification system works")
    )
    .toJSON();
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isChannelSelectMenu()) {
      if (interaction.customId === "deploy_verify_channel") {
        await handleDeployChannelSelect(interaction as ChannelSelectMenuInteraction);
      } else if (["vp_requests_channel", "vp_logs_channel"].includes(interaction.customId)) {
        await handleVerifyPanelSelect(interaction as ChannelSelectMenuInteraction);
      }
      return;
    }
...
async function handleDeployChannelSelect(interaction: ChannelSelectMenuInteraction) {
