...
interface AutoRoleState {
  memberRoleId?: string;
  botRoleId?: string;
}

export const autoRoleState = new Map<string, AutoRoleState>();

function buildAutoRoleEmbed(state: AutoRoleState) {
  return new EmbedBuilder()
    .setColor(0x5000ff)
    .setTitle("⚙️ Auto-Role Setup")
    .setDescription(
      "Configure roles to automatically assign when someone joins the server.\n" +
...
function buildAutoRoleComponents(state: AutoRoleState) {
...
export async function openAutoRolePanel(interaction: ButtonInteraction) {
...
export async function handleAutoRoleSave(interaction: ButtonInteraction) {
  const userId = interaction.user.id;
  const state = autoRoleState.get(userId) ?? {};
...
export async function handleAutoRoleClear(interaction: ButtonInteraction) {
  const guildId = interaction.guild!.id;
