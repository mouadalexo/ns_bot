import { EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, RoleSelectMenuBuilder, ChannelSelectMenuBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ChannelType, } from "discord.js";
import { db } from "@workspace/db";
import { botConfigTable } from "@workspace/db";
import { eq } from "drizzle-orm";
export const verifyPanelState = new Map();
...
function buildVerifyPanelEmbed(state) {
    const lines = [
...
function buildVerifyPanelComponents(state) {
    ...
    const row2 = new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder()
        ...
        .setMinValues(1).setMaxValues(1));
    const row3 = new ActionRowBuilder().addComponents(new RoleSelectMenuBuilder()
        .setCustomId("vp_roles_group")
        .setPlaceholder([state.verifiedRoleId && "Verified", state.unverifiedRoleId && "Unverified", state.jailRoleId && "Jail"]
    ...
    const row4 = new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder()
        .setCustomId("vp_logs_channel")
        .setPlaceholder(state.logsChannelId ? "Logs Channel (set)" : "Logs Channel (outcomes, optional)...")
        .addChannelTypes(ChannelType.GuildText)
        .setMinValues(0).setMaxValues(1));
    const row5 = new ActionRowBuilder().addComponents(new ButtonBuilder()
        .setCustomId("vp_save")
        .setLabel("Save")
        .setStyle(ButtonStyle.Success)
        .setDisabled(!canSave), new ButtonBuilder()
...
export async function openVerifyPanel(interaction) {
...
export async function openEditQuestionsModal(interaction) {
...
export async function handleEditQuestionsSubmit(interaction) {
    const guildId = interaction.guild.id;
    const questions = [
...
export async function openEmbedCustomizeModal(interaction) {
    ...
    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder()
        ...
        .setRequired(true)
        .setMaxLength(100)
        .setValue(state.embedTitle ?? "Night Stars — Verification")), new ActionRowBuilder().addComponents(new TextInputBuilder()
        ...
        .setValue(state.embedDescription ??
...
export async function handleEmbedCustomizeSubmit(interaction) {
    ...
    const backRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
...
export async function handleEmbedPreviewBack(interaction) {
    const state = verifyPanelState.get(interaction.user.id) ?? {};
...
export async function handleVerifyPanelSelect(interaction) {
    const userId = interaction.user.id;
    const state = verifyPanelState.get(userId) ?? {};
    if (interaction.customId === "vp_verificators_role") {
        state.verificatorsRoleId = interaction.values[0];
    }
    else if (interaction.customId === "vp_requests_channel") {
        state.requestsChannelId = interaction.values[0];
    }
        ...
        if (values.length >= 1)
            state.verifiedRoleId = values[0];
        if (values.length >= 2)
            state.unverifiedRoleId = values[1];
        if (values.length >= 3)
            state.jailRoleId = values[2];
    }
    verifyPanelState.set(userId, state);
    await interaction.update({
        embeds: [buildVerifyPanelEmbed(state)],
...
export async function handleVerifyPanelSave(interaction) {
    const userId = interaction.user.id;
    const state = verifyPanelState.get(userId) ?? {};
...
export async function handleVerifyPanelReset(interaction) {
    const state = {};
